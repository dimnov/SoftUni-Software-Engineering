function rosettaStone(input) {
  let rowsTemplate = Number(input[0]);
  let templateMatrix = [];
  let templateCol = 0;
  let counter = 0;
  ...
  let messageMatrix = [];
  let messageRows = input.length - rowsTemplate - 1;
  let messageCol = 0;

  for (let i = rowsTemplate + 1; i < input.length; i++) {
    counter++;
    messageMatrix.push(input[i].split(' ').map(Number));
  }
